      ...
      {/* <h1>{data[0]["price"]}</h1> */}
        ...
        {dataa.map((item, key) => (
          <div className='raw-table' style={{"backgroundColor": deleted.filter((item1) => item["microcategory_id"] == item1["microcategory_id"] && item["region_id"] == item1["region_id"]).length >= 1 ? "#f7d6e6" : changes.filter((item1) => item["microcategory_id"] == item1["microcategory_id"] && item["region_id"] == item1["region_id"]).length >= 1 ? "#fff3cd"  : ""}}>
              ...
              {changes.filter((item1) => item["microcategory_id"] == item1["microcategory_id"] && item["region_id"] == item1["region_id"]).length >= 1 ? <img className='crosses-img' src={grayPlus}/> : <div style={{"width": 29}}/>}
              {/* <div className='empty-div'></div> */}
              <div className='icons-div'>
                <img onClick={() => deleted.filter((item1) => item["microcategory_id"] == item1["microcategory_id"] && item["region_id"] == item1["region_id"]).length >= 1 ? handleAddBack(key) : handleDelete(key)} src={deleted.filter((item1) => item["microcategory_id"] == item1["microcategory_id"] && item["region_id"] == item1["region_id"]).length >= 1 ? grayPlus : redCross} className='crosses-img'></img>
              </div>
          </div>
        ))}
    </div>
